...
impl<'a> PaneContentsAndUi<'a> {
    ...
    pub fn render_fake_cursor_if_needed(&mut self, client_id: ClientId) -> Result<()> {
        ...
        if pane_focused_for_different_client && !pane_focused_for_client_id {
            ...
            if let Some(colors) =
                client_id_to_colors(*fake_cursor_client_id, self.style.theme.palette.into())
            {
            ...
            match mode {
                InputMode::Normal | InputMode::Locked => {
                    if session_is_mirrored || !self.multiple_users_exist_in_session {
                        let colors = self.style.theme.styling.selected_frame.fg;
                        Some(colors)
                    } else {
                        let colors =
                            client_id_to_colors(client_id, self.style.theme.palette.into());
                        colors.map(|colors| colors.0)
                    }
                },
                _ => Some(self.style.theme.styling.selected_frame.fg),
            }
        } else {
            None
        }
